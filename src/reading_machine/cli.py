...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Reading-Machine thinks for you about the meanings.",
        epilog="Example:  reading-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | reading-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | reading-machine         #

    $ reading-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="reading-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"reading-machine: {utterance}.")
        syslog.closelog()
